...
add_heap(size_t req_size)
{
    void *p;
    Header *align_p;
    // printf("gcheap:%zu %d %zu %d\n", gc_heaps_used,HEAP_LIMIT,req_size,TINY_HEAP_SIZE);
...
// 判断对象是否为指向堆上的指针，是否已分配，若满足则继续trace
static void
gc_mark(void * ptr)
{
    GC_Heap *gh;
...
gc_mark_register(void)
{
    ...
    setjmp(env); // 保存当前寄存器的值
    // printf("gc_mark_register:%d\n",sizeof(env));
...
garbage_collect(void)
{
    ...
    for (i = 0; i < root_ranges_used; i++) {
        printf("garbage_collect:marking roots\n");
...
test_mini_gc_malloc_free(void)
{
    ...
    // printf("%p\n", ((Header *)p1-1)->next_free);
    printf("(p2-1)-p3=%lu\n",(size_t)((Header *)p2-1)-(size_t)(p3));
...
printfreelist()
{
    if (free_list!=NULL)
    {
        printf("free_list=%p %p %p free_list->size=%zu\n", free_list,free_list->next_free,free_list->next_free,free_list->size);
    } else {
...
/*
内存管理及垃圾收集学习总结：
1:通常由Heap统一管理向操作系统申请的内存，且向操作系统申请内存时，一次性申请一大块的内存；
2:当程序需要内存分配对象时，小对象《 heap size 用尝试在Heap中分配，若heap空间不足，则执行1，分配完后，整理freelist链表
大对象直接向操作系统申请合适内存
3:mark sweep 算法： trace 寄存器、栈、ROOT上的值，分析是否为指向堆上的已分配对象地址空间，若是 则打上标记
4:清除扫描heap上对象块，对标记对象进行释放，尝试合并且最终放到free_list链表上去
*?
